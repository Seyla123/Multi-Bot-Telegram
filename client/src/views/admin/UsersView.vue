  ...
  <div class="min-h-screen bg-slate-50/50 p-6 md:p-10">
    <div class="max-w-7xl mx-auto space-y-6">
      ...
      <CrudTable
        ...
        subtitle="Manage administrator access privileges and support agent accounts."
        :columns="columns"
        :items="items"
...
const columns: CrudColumn[] = [
  ...
  { 
    ...
    label: 'Role', 
    type: 'badge',
    badgeStyle: (val) => String(val).toUpperCase() === 'ADMIN' ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-blue-50 text-blue-700 border-blue-200'
  },
...
const fields = computed<CrudField[]>(() => {
  const baseFields: CrudField[] = [
    { name: 'name', label: 'Full Name', type: 'text', required: true, placeholder: 'e.g. Alexander Wright' },
    { name: 'email', label: 'Email Address', type: 'email', required: true, placeholder: 'alexander@company.com' },
    { 
      name: 'role', 
      label: 'System Role', 
      type: 'select', 
...
const openCreateDrawer = () => {
  ...
  currentFormItem.value = { role: 'AGENT' };
...
const handleDelete = async (id: number | string) => {
  if (confirm('Are you sure you want to delete this user? They will lose dashboard access immediately.')) {
    await deleteItem(id);
  }
};
